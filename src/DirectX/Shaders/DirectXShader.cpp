...
namespace dx
{
  const void* CompileShader(const std::string filepath, SIZE_T &size)
  {
    ...
    if (filepath.length() < fileSuffix.length() and not std::equal(fileSuffix.rbegin(), fileSuffix.rend(), filepath.rbegin()))
    {
      return nullptr;
    }
    ...
    if (FAILED(D3DCompileFromFile(stemp.c_str(), nullptr, nullptr, "main", shaderType.c_str(), D3DCOMPILE_ENABLE_STRICTNESS, 0, &shaderBlob, &errorBlob)))
    {
